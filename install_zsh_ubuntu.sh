...
# Updates default zsh theme
if [ -f "$ZSHRC" ]; then
  # Substitui a linha começada por "ZSH_THEME=" pela nova linha com "powerlevel10k/powerlevel10k"
  sed -i 's/^ZSH_THEME=.*/ZSH_THEME="powerlevel10k\/powerlevel10k"/' "$ZSHRC"
else
  echo "~/.zshrc not found!!!"
fi

# Zsh plugins (completions, autosugestions and syntax-hightlighting)
...
if [ -f "$ZSHRC" ]; then
  # Substitui a linha começada por "plugins=" pela nova linha
...
echo "Restart termianl and run 'p10k configure'"
